...
2. 새로운 브랜치 생성 및 이동:
   ```bash
   # 참고: 브랜치의 이름은 작업 영역에 따라 구분되도록 작성합시다.
...
4. 브랜치 삭제:
   ```bash
   # Pull Request(PR)가 승인되어, 브랜치가 잘 병합되었다면, 해당 브랜치를 삭제합시다.
   git branch -d <브랜치이름>
   ```
5. 로그 확인:
   ```bash
...
## 5. 간단한 협업 흐름 설명
1. main 브랜치에서 새로운 작업 브랜치 생성 (feature/, bugfix/ 등, 작업 내용이 드러나는 이름!).
2. 작업 브랜치에서 개발하고 커밋.
3. (필요한 경우) main 브랜치에서 최신 내용을 가져와 자신의 브랜치에 포함시킬 것. (git pull origin main)
4. 작업이 완료되면 PR을 생성하여 코드 리뷰 요청.
5. 리뷰가 완료되면 main 브랜치에 병합. (PR 승인)
6. 병합 후 브랜치 삭제. (제가 병합을 승인하며 동시에 삭제하겠습니다.)

## 6. 주의사항
- 작업 전 항상 `git pull origin main`으로 최신 상태를 불러오세요.
- 주기적으로 `git status`를 확인하여 변경사항을 관리하세요.
- 무엇을 변경했는지에 대하여, 명확한 커밋 메시지를 작성하세요.
- main 브랜치로 바로 push하지 말아주세요.
   - 모든 작업자는 작업 내용이 드러나는 이름으로 브랜치를 생성합시다.
...
- Docker 환경 구축 이후 작업 과정
   ...
   git branch  # 자신의 브랜치로 이동되었는지 확인하세요.
      ...
      # 만약 자신의 브랜치가 아니라면, 브랜치를 생성하거나, 자신의 브랜치로 이동하세요.
   ...
   docker compose up -d --build  # docker 컨테이너 실행. -d 옵션은 백그라운드 실행입니다. --build 옵션은 기존 이미지를 파기하고 새롭게 빌드합니다.
   ...
   # 이제 병합이 완료되었다면, 브랜치를 삭제하십시오! (제가 병합을 승인하며 GitHub에서 삭제하겠습니다.)

   ------

   # 이제 작업이 종료되었습니다. 그러나 '로컬 깃'에는 여전히 브랜치가 있습니다. 해당 브랜치를 삭제해 봅시다.
   git checkout main  # 자신의 브랜치를 삭제하기 위하여, main으로 이동합니다.

   git branch -d <삭제할 브랜치 이름>  # 로컬 브랜치를 삭제합니다. (병합이 완료되었다면, 경고 메시지를 무시하고 -D 옵션을 사용해도 됩니다.)


   # 만일 GitHub에 여전히 쓸모없는 브랜치가 남아있다면, 원격으로 삭제할 수 있습니다.
   git push origin --delete <로컬에서 삭제한 브랜치 이름>  # 원격 브랜치를 삭제합니다.


   # 이제 다음 작업을 수행하십시오! 새로운 브랜치를 생성하고, 작업하세요!
   ```
