...
export default function Code200Page() {
  ...
  const leadingPrompt =
    "Analyze the logs with status codes ranging from 200 to 299 and generate a security report about the server status within 100 words.";

  useEffect(() => {
    async function fetchData() {
      const filteredLogs = await fetchFilteredLogs("2");
    ...
    <div>
      <StatusLayout>
        <div className="flex justify-between items-center mb-4">
          <h1 className="text-2xl font-bold">Status Code 200-299</h1>
          <button
