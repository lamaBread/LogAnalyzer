...
export let sortedLogs:
  | { normal_access: string; probable_attack: string; suspicious_activity: string }
  | null = null;
...
Logs:\n${JSON.stringify(logs)}`;
    ...
    if (!response.ok) throw new Error("Failed to fetch structured response");

    const structuredResponse = await response.json();

    sortedLogs = structuredResponse;
